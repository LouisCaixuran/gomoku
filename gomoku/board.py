...
class Gomoku(object):
    ...
    def play(self, player1, player2, isShow=False):
        ...
        while True:
            ...
            self.last_action = self.get_current_player(player1,
                                                       player2).get_action()
            ...
            self.get_current_player(player1,
                                    player2).reply(
    ...
    def showboard(self):
        ...
        if self.last_action != -1:
            print("player1: O, last action:",
                  self.player1_last_action // self.height,
                  self.player1_last_action % self.width)
            print("player2: X, last action:",
                  self.player2_last_action // self.height,
                  self.player2_last_action % self.width)
    ...
    def is_won(self):
        ...
        for m in player_moved:
            ...
            if (h in range(self.height - n + 1) and
                len(set(self.status[i] for i in range(m,
                                                      m + n * self.width,
                                                      self.width))) == 1):
                return True

            if (w in range(self.width - n + 1) and
                h in range(self.height - n + 1) and
                len(set(self.status[i] for i in range(m,
                                                      m + n * (self.width + 1),
                                                      self.width + 1))) == 1):
                return True

            if (w in range(n - 1, self.width) and
                h in range(self.height - n + 1) and
                len(set(self.status[i] for i in range(m,
                                                      m + n * (self.width - 1),
                                                      self.width - 1))) == 1):
    ...
    def current_state(self):
        ...
        if self.last_action != -1:
            state[2,
                  self.last_action // self.width,
                  self.last_action % self.width] = 1
        return state
