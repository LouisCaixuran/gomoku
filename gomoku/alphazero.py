...
class TreeNode(object):
    ...
    def get_value(self, c_puct):
        return self._Q + self._P * c_puct * \
            np.sqrt(np.log(self._parent._N) / (self._N + 1e-3))
...
class MCTS(object):
    ...
    def _selection(self, board):
        ...
        while not node.is_leaf():
            ...
            action, node = max(node._children.items(),
                               key=lambda act_node:
                               act_node[1].get_value(self._c_puct))
    ...
    def get_move_probs(self, board, temperature=1e-3):
        ...
        act_probs = self.soft_max(
            1.0 / temperature * np.log(np.array(visits) + 1e-10))
